...
import { authRedirectBase, signInWithApple } from "@/lib/auth";
...
export default function Auth() {
  ...
  const handleApple = async () => {
    ...
    try {
      ...
      // No nativo a sessão já foi criada aqui; o web volta via redirect.
      navigate("/");
    } catch (err) {
      const msg = err instanceof Error ? err.message : "Erro ao entrar com a Apple";
      // O usuário cancelar o prompt não é um erro a ser exibido.
      if (!/cancel|canceled|1001/i.test(msg)) toast.error(msg);
    } finally {
      ...
      <div className="safe-top safe-bottom relative mx-auto flex min-h-dvh w-full max-w-md flex-col justify-center gap-8 px-6">
        ...
        >
          ...
          <button
            ...
            className="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-white text-base font-semibold text-black transition hover:bg-white/90 disabled:opacity-60"
          >
            {appleLoading ? (
              "Aguarde…"
            ) : (
